...
const ManageProductsClient: React.FC<ManageProductsClientProps> = ({
  ...
  const handleToggleStock = useCallback((id: string, inStock: boolean) => {
    axios
      ...
      .then((res) => {
        toast.success("Existencia del producto cambiada");
        router.refresh();
      })
      .catch((err) => {
  ...
  const handleDelete = useCallback(async (id: string, images: any[]) => {
    toast("Borrando producto, por favor espera!", {
      icon: "🔃",
    });
      ...
      try {
        for (const item of images) {
          if (item.image) {
            ...
            console.log("Imagen borrada correctamente.", item.image);
          }
        }
      } catch (err) {
    ...
    axios
      ...
      .catch((err) => {
        toast.error("Oops! Error al borrar producto");
        console.log(err);
      });
  }, []);
